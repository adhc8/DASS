...
class engage_target:
    ...
    def target_dist(self):
        if self.target_info[2] != 0:
            target_distance = self.target_info[2]
        elif self.target_info[0] != 0:
            width = self.target_info[0] # in cm
            target_distance = width*self.target_info[3]/const_cm_pix
        else:
            guess_width = 20 # in cm
            target_distance = guess_width*self.target_info[3]/const_cm_pix
    ...
    def calc_x_velocity(self):
        if self.target_info[0] == 0:
            ...
            x_vel = (round((self.target_info[0]-x_centre)/2,3)*100)/const_cm_pix # m/s to one decimal
        return x_vel
    ...
    def calc_y_velocity(self):
        if self.target_info[1] == 0:
            ...
            y_vel = (round((self.target_info[1]-y_centre)/2,3)*100)/const_cm_pix #m/s to one decimal
        return y_vel
